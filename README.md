...
## Run in Docker
The APIs were meant to run in Docker. Run `$ make containers` to build the containers and run `$ make run` to run them. **Note:** You do need to follow the installation steps described below, before the containers can be built. Specifically, the part about configuration.
